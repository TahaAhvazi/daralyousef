...
export const BoardOrderCard = memo(function BoardOrderCard({
  order,
  onAdvance,
  onRevert,
  loading,
  ...
  const codeEl = canOpenDetail ? (
    <Link
      ...
      className={cn(
        ...
        touchMode ? "text-[15px]" : compact ? "text-[12px]" : "text-[13px]",
      )}
  ...
  if (compact && !touchMode) {
    return (
      <div
        ...
        className={cn(
          "flex h-full min-w-0 flex-col select-none overflow-hidden rounded-lg border border-border border-s-[3px] bg-surface p-2 transition-shadow hover:shadow-md",
          border,
          canDrag && "cursor-grab active:cursor-grabbing",
          isDragging && "opacity-40 ring-2 ring-brand/30",
          overdue && "bg-danger/[0.04]",
          mine && !overdue && "bg-brand/[0.04]",
        )}
      >
        <div className="flex min-w-0 flex-1 flex-col">
          <div className="flex items-start gap-1">
            {canDrag ? (
              <GripVertical className="mt-0.5 size-3 shrink-0 text-text-3 opacity-50" aria-hidden />
            ) : null}
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1">
                <div className="min-w-0 flex-1">{codeEl}</div>
                {mine ? (
                  <Badge variant="brand" className="!text-[9px] !px-1 !py-0 shrink-0">
                    {bb.mineBadge}
                  </Badge>
                ) : null}
                {overdue ? (
              ...
              {order.order_title ? (
                <p className="mt-0.5 truncate text-[11px] font-medium leading-snug text-text-2">
                  {order.order_title}
                </p>
              ) : null}
            </div>
          ...
          <div className="mt-1.5 flex items-center gap-1.5">
            <div className="h-1 min-w-0 flex-1 overflow-hidden rounded-full bg-surface-2">
              <div
                ...
                style={{ width: `${Math.max(0, Math.min(100, order.progress_pct))}%` }}
              />
            </div>
            <span className="shrink-0 text-[10px] tabular-nums text-text-3">{order.progress_pct}%</span>
            <Badge variant={priorityVariant} className="!text-[9px] !px-1 !py-0 shrink-0">
              {priorityLabel}
            </Badge>
          </div>

          <div className="mt-1.5 flex items-center justify-between gap-1 text-[10px] text-text-3">
            <span
              className={cn(
            ...
            >
              ...
              <span className="truncate">
                {order.order_deadline ? formatDate(order.order_deadline) : bb.noDeadline}
              </span>
            </span>
            <span className="inline-flex min-w-0 max-w-[45%] items-center gap-0.5">
        ...
        {(canAdvance || canRevert || canOpenDetail) && (
          <div className="mt-auto flex items-center gap-1 pt-1.5">
            {nextCol && canAdvance ? (
              <Button
                ...
                loading={loading}
                title={columnLabels[nextCol] ?? nextCol}
            ...
            {canOpenDetail ? (
              <Link to={`/app/orders/${order.order_id}`} className="shrink-0">
                <Button size="sm" variant="ghost" className="!h-6 !px-1.5" title={bb.openOrder}>
  ...
  /* ── Touch / expanded card ────────────────────────────────────────────── */
  const sourceLabel =
    order.placed_via === "portal"
      ? bb.sourcePortal
      : order.placed_via === "daftra" || order.daftra_id
        ? bb.sourceDaftra
        : bb.sourceStaff;
    ...
    >
      <div className="mb-2 flex items-start justify-between gap-2">
        <div className="min-w-0 flex-1">
          ...
          {order.order_title ? (
            <p className={cn("mt-0.5 truncate font-medium text-text", touchMode ? "text-[13px]" : "text-[12px]")}>
              {order.order_title}
            </p>
          ) : null}
        </div>
      ...
      <div className="mb-2">
        <div className="mb-1 flex items-center justify-between text-[11px] text-text-3">
          <span>{bb.progressLabel}</span>
          <span className="tabular-nums font-medium text-text-2">{order.progress_pct}%</span>
        </div>
      ...
      {order.items_summary ? (
        <p className={cn("line-clamp-2 leading-relaxed text-text-2", touchMode ? "text-[12.5px]" : "text-[11.5px]")}>
          {order.items_summary}
        </p>
      ) : null}

      {(WORKFLOW_BOARD_STAGES as readonly string[]).includes(order.board_column) ? (
      ...
      <div className="mt-2.5 flex flex-wrap items-center gap-1.5">
        ...
        <span className={cn("rounded-md bg-surface-2 px-1.5 py-0.5 text-text-3", touchMode ? "text-[11px]" : "text-[10px]")}>
          {sourceLabel}
        </span>
      </div>

      <div className={cn("mt-2.5 flex items-start justify-between gap-2 text-text-2", touchMode ? "text-[12px]" : "text-[11px]")}>
        <div className="min-w-0 space-y-1">
          <span className={cn("inline-flex items-center gap-1", overdue && "font-semibold text-danger")}>
            <Calendar className="size-3.5 shrink-0" />
            <span className="truncate">
              {order.order_deadline ? `${bb.dueLabel}: ${formatDate(order.order_deadline)}` : bb.noDeadline}
            </span>
          </span>
